...
import { getNoteRecord, loadWorkbenchState, upsertNoteRecord } from '@/lib/wrong-workbench-server-store'
...
export async function GET(req: NextRequest) {
  ...
  if (wrongId && questionId) {
    const item = await getNoteRecord(userId, wrongId, questionId)
    return NextResponse.json({ item })
  }

  const state = await loadWorkbenchState(userId)
  return NextResponse.json({ items: state.notes })
}
...
export async function POST(req: NextRequest) {
  ...
  const record = await upsertNoteRecord(userId, {
