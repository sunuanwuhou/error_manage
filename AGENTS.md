...
- 真题来源名不能直接把原文件名原样入库。
  - 必须拆成稳定的 session label 与标题本体
  - 默认格式收敛为“年份 + 考试类型/地区 + 标题本体”
  - 套卷卡片标题只显示标题本体，避免年份/地区/考试类型重复堆叠
- 图片题展示规则必须跨页面保持一致。
...
- 真题导入验收不能再只靠人工抽样。
  - 至少要支持一条脚本化自动对账链路
  - 对账项至少包含：总题数、题号缺失、题型、答案、选项、图片入库数
- 套卷模块分段不能只看总题型计数。
  - 必须额外核对题型是否按题号连续成段
  - 若页面出现 `7 / 1 / 6` 这类碎块，优先回查解析层题型错分，不要先用前端合并掩盖
